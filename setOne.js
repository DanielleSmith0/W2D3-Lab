...
const myChoice = (anything) => console.log(`${anything}, great choice!`)
...
//Write inner(callback fxn first)

const fullName = (firstName, lastnName) => {
    console.log("Hello, my full name is " + firstName + " " + lastnName)
}

// Outer fxn that returns inner fxn
...
greeting("Danielle", "Smith", fullName);
...
// inner fxn - loop over each variable and it to 20% of itself. Push the answer to the totalCost array.


//outer fxn

const pricesPlusTax = (oldPrice, callback) => {
    for(let i = 0; i < oldPrice.length; i++) {
        totalCost.push(oldPrice[i] + oldPrice[i] * .2)
    }
    callback(totalCost);
}
